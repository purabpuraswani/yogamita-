...
function findDatasetForStep(stepKey) {
	const directPath = path.join(DATASETS_DIR, `${stepKey}_dataset.json`);
	...
	const candidates = fs
		...
		.filter((entry) => entry.isFile() && (entry.name === `${stepKey}_dataset.json` || entry.name.endsWith(`_${stepKey}_dataset.json`)))
	...
	if (candidates.length > 1) {
		throw new Error(
			`Multiple datasets matched for ${stepKey}: ${candidates.join(', ')}. Keep only one or use ${stepKey}_dataset.json.`
		);
	}
