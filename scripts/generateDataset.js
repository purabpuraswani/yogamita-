...
function buildOutputFileName(poseName, stepName) {
	return `${sanitizeName(stepName)}_dataset.json`;
}
...
function resolveDatasetStepsDir() {
	...
	throw new Error(
		...
		`Pass a custom path as first argument, e.g. node scripts/generateDataset.js ./dataset_steps`
	);
}
...
async function buildStepwiseDatasets() {
	...
	for (const poseName of poseDirs) {
		...
		for (const stepName of STEP_FOLDERS) {
			...
			const outputFile = path.join(OUTPUT_DIR, sanitizeName(poseName), buildOutputFileName(poseName, stepName));
